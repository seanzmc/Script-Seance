...
export const VoiceManager: React.FC<VoiceManagerProps> = ({
  ...
  const renderRow = (char: string, options?: { variant?: 'narrator' | 'cast'; index?: number }) => {
    ...
    const rowTone = isNarrator
      ...
      : isEvenRow
        ? 'border-gray-800/90 bg-gray-950/35'
        ...
        <div className={`space-y-2 rounded-lg border px-2.5 py-2 ${rowTone} ${isActive ? 'ring-1 ring-indigo-500/40' : ''}`}>
          <div className="flex items-start justify-between gap-2">
            ...
            {isThisLoading || isThisPlaying ? (
              <span className="shrink-0 rounded-md border border-red-500/50 bg-red-500/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.16em] text-red-200">
                {isThisLoading ? 'Loading' : 'Previewing'}
              </span>
            ) : null}
          </div>
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
            <select
              ...
              className="min-w-0 w-full bg-gray-900/80 border border-gray-700 text-gray-200 text-xs rounded-md px-2.5 py-2 focus:ring-1 focus:ring-indigo-500 outline-none appearance-none"
            >
            ...
            <button
              ...
              className={`h-8 px-2.5 rounded-md border text-[11px] font-semibold transition-colors inline-flex items-center gap-1 whitespace-nowrap ${
                previewDisabled
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <button
              ...
              aria-label="Cast"
              className="px-2 py-1.5 rounded-md border text-[11px] font-semibold transition-colors inline-flex items-center gap-1.5 whitespace-nowrap border-transparent text-gray-400 hover:text-indigo-200 hover:border-gray-700 hover:bg-gray-800/50"
              title="Cast"
            >
            ...
            <button
              ...
              className={`px-2 py-1.5 rounded-md border text-[11px] font-semibold transition-colors inline-flex items-center gap-1.5 whitespace-nowrap ${
                isExpanded
                  ? 'bg-gray-800/70 border-gray-700 text-indigo-200'
                  : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-700 hover:bg-gray-800/50'
              }`}
          ...
          {isExpanded && (
            <div
              ...
              className="grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-md border border-gray-800/80 bg-gray-900/25 px-2 py-2"
            >
              <div>
                <div className="mb-1 flex items-center justify-between gap-2">
                  <label className="text-[9px] uppercase font-bold text-gray-500 tracking-wider">Speed</label>
                  <span className="rounded bg-gray-800 px-1 text-[10px] text-gray-300">{config.speed?.toFixed(1) || '1.0'}x</span>
                </div>
                <input
                  type="range"
                  min="0.5"
              ...
              <div>
                <div className="mb-1 flex items-center justify-between gap-2">
                  ...
                  <span className="rounded bg-gray-800 px-1 text-[10px] text-gray-300">{config.pitch || 0}</span>
                </div>
                <input
                  type="range"
                  min="-5"
